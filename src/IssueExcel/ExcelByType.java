...
public class ExcelByType implements ByType {
	private static String fileLocation;
	private HashMap<String, Integer> columnMap;
	private String version1;
	private String version2;
	
	public ExcelByType(String location, String version1, String version2) {
		...
		try {
			...
			for(int i = 1; i <= rowLen; i++) {
				...
				if(compareVersion(version, version1) > 0 && compareVersion(version, version2) <= 0) {
					...
					String issueType = sheet.getRow(i).getCell(typeIndex).toString();
